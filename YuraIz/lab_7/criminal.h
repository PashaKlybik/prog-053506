...
void killCriminal(Criminal* criminal) {
...
typedef struct CriminalList CriminalList;

struct CriminalList {
        List* data;
        void (*add)(CriminalList*);
        Criminal* (*elementAt)(CriminalList*, int index);
        //void (*removeByName)();
};

void _criminalListAdd(CriminalList* criminalList) {
        criminalList->data->add(criminalList->data, addCriminal());
}

CriminalList* initCriminalList() {
        CriminalList* ptr = malloc(sizeof(CriminalList));
        ptr->data = initList();
        ptr->add = _criminalListAdd;
        //ptr->elementAt = (Criminal * (*) (CriminalList *, int))ptr->data->elementAt;
        return ptr;
}
