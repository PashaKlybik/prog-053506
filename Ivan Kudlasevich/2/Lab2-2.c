...
{
    double res = a / b * znak;
    if (fabs(res) < eps) return res;
    else return res + sinRec(x, eps, a * x * x, b * (b + 1) * (b + 2), znak * -1);
}
...
void f(double x, double eps)
{
    ...
    printf("Eps %f\n", eps);
    if (iteration != mxIteration) printf("Need %d to reach eps\n", iteration);
    else printf("Need more then %d to reach eps\n", iteration);
