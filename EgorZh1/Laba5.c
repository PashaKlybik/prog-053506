...
Number createNumber(char numberValue[])
{
    ...
    for(int i = strlen(numberValue)-1; i >= 0; i--) addDigit(&number, numberValue[i]-'0');
...
Number longSumLong(Number firstNum, Number secondNum, int fromFunc)
{
    ...
    if(firstNum.isNeg && secondNum.isNeg) sum.isNeg = 1;
    ...
    if(remainder) addDigit(&sum, remainder);
...
Number longSubLong(Number firstNum, Number secondNum, int fromFunc)
{
    ...
    if(firstNum.length > secondNum.length) firstIsBigger = 1;
    else if(firstNum.length < secondNum.length) firstIsBigger = 0;
         else
         {
             first = firstNum.tail;
...
Number longMulLong(Number firstNum, Number secondNum, int fromFunc)
{
    ...
    for(int i = 0; i < secondNum.length; i++)
    {
        ...
        if(i)
        {
            for(int j = 0; j < i; j++) addDigit(&number, 0);
